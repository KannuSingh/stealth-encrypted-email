...
describe('StealthKeyRegistry', () => {
  ...
  beforeEach(async () => {
    ...
    const stealthKeyRegistryArtifact = await ethers.getContractFactory("StealthKeyRegistry"); 
    registry = (await stealthKeyRegistryArtifact.deploy()) as StealthKeyRegistry;
  });
  ...
  describe('initialization', () => {
    ...
    it('should initialize with empty stealth keys', async () => {
      ...
      expect(spendingPubKey).to.equal(0);
      expect(viewingPubKey).to.equal(0);
    });
  });
  ...
  describe('set stealth keys directly', () => {
    it('should let the user set their stealth keys', async () => {
      ...
      expect(spendingPubKeyPrefix).to.equal(2);
      expect(spendingPubKey).to.equal(SPENDING_KEY);
      expect(viewingPubKeyPrefix).to.equal(2);
      expect(viewingPubKey).to.equal(VIEWING_KEY);
    });
    ...
    it('should let the user overwrite existing stealth keys', async () => {
      ...
      expect(spendingPubKeyPrefix).to.equal(3);
      expect(spendingPubKey).to.equal(SPENDING_KEY);
      expect(viewingPubKeyPrefix).to.equal(2);
      expect(viewingPubKey).to.equal(VIEWING_KEY);
      ...
      expect(spendingPubKeyPrefix).to.equal(2);
      expect(spendingPubKey).to.equal(SPENDING_KEY + 1);
      expect(viewingPubKeyPrefix).to.equal(3);
      expect(viewingPubKey).to.equal(VIEWING_KEY + 1);
    });

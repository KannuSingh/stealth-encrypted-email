...
import { StealthKeyRegistry as StealthKeyRegistryContract } from '../typechain/StealthKeyRegistry';
...
export class StealthKeyRegistry {
  ...
  async getStealthKeys(account: string) {
    ...
    if ((spendingPubKeyPrefix as any).eq(0) || (spendingPubKey as any).eq(0) || (viewingPubKeyPrefix as any).eq(0) || (viewingPubKey as any).eq(0)) {
      throw new Error(`Address ${account} has not registered stealth keys. Please ask them to setup their E3 account`); // prettier-ignore
    }

    // Decompress keys and return them
    const spendingPublicKey = KeyPair.getUncompressedFromX(spendingPubKey, spendingPubKeyPrefix.toString());
    const viewingPublicKey = KeyPair.getUncompressedFromX(viewingPubKey, viewingPubKeyPrefix.toString());
    return { spendingPublicKey, viewingPublicKey };
  }
  async isStealthKeysSet(account: string) {
    // Read stealth keys from the resolver contract
    console.log(`In isStealthKeysSet method to check whether ${account} have already registered stealth keys.`)
    const keys = await this._registry.stealthKeys(account);
    const { spendingPubKeyPrefix , spendingPubKey, viewingPubKeyPrefix, viewingPubKey } = keys;
